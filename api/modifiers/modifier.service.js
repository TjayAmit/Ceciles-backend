...
function getalloproduct( product_id, modi,callback) {
      var sql = `SELECT * FROM suggested_order WHERE product_id = "${product_id}"`;
      pool.query(sql, function(err, results){
...
function getallobranch( branch,modi, callback) {
      var sql = `SELECT * FROM suggested_order WHERE branch = "${branch}"`;
      pool.query(sql, function(err, results){
...
function getbranchproduct( branch, product, modi, callback) {
      var sql = `SELECT * FROM suggested_order WHERE branch = "${branch}" AND product_id ="${product}"`;
      pool.query(sql, function(err, results){
...
function getsuggestedallo(modi,callback) {
      var sql = `SELECT * FROM suggested_order`;
      pool.query(sql, function(err, results){
...
function sap(productid,ts, callback) {
      var sql = `SELECT * FROM suggested_order WHERE product_id = "${productid}"`;
      pool.query(sql, function(err, results){
            ...
            if(date_ob >= startdate  || date_ob <= enddate ){
              ...
              pool.query(query,
                (error,results2) => {
                  ...
                  pool.query(`SELECT s.product_id, sum(s.suggested_allocation_quantity) AS total_sa,SUM(i.quantity) AS total_quantity FROM suggested_order s
                        ...
                        for(var i = 0; i < data.length; i++){
                            ...
                            pool.query(`UPDATE suggested_order s SET percentage_quantity = s.suggested_allocation_quantity/"${data[i].total_sa}" WHERE s.product_id = "${data[i].product_id}"`,
                                ...
                                if(datarecord == datalength){
                                    return callBack(null,'Success in importing suggested_order')
                                }
                              });
                            }   
                          }else{
                            ...
                            if(branch === "ALL" && product === "ALL"){
                                        getsuggestedallo(modi,function(result,m){
                                          ...
                                          for(var n = 0; n < data1.length; n++){
                                            ...
                                            var r = ((sq * m) + sq);
                                            pool.query(`UPDATE suggested_order SET suggested_allocation_quantity=${r} WHERE suggested_id = ${data1[n].suggested_id}`, (err, results, fields) => {
                                              if (err) {
                                        ...
                                        getalloproduct(product,modi, function(result,m){
                                          ...
                                          for(var f = 0; f < data2.length; f++){
                                            var sq = data2[f].suggested_allocation_quantity;
                                             var r = ((sq * m) + sq);
                                            pool.query(`UPDATE suggested_order SET suggested_allocation_quantity=${r} WHERE suggested_id = ${data2[f].suggested_id}`, (err, results, fields) => {
                                              if (err) {
                                                  return res.status(500).json({err});
                                              }
                                               console.log("success");
                                        ...
                                        getallobranch(branch, modi, function(result,m){
                                          ...
                                          console.log("situation 3  " +data3.length);
                                            ...
                                            var sq = data3[n].suggested_allocation_quantity;
                                             var r = ((sq * m) + sq);
                                            pool.query(`UPDATE suggested_order SET suggested_allocation_quantity=${r} WHERE suggested_id = ${data3[n].suggested_id}`, (err, results, fields) => {
                                              if (err) {
                                                  return res.status(500).json({err});
                                              }
                                            console.log("success");
                                      ...
                                      if(product != "ALL" && branch != "ALL"){
                                        getbranchproduct(branch,product,modi,function(result,m){
                                            ...
                                            var sq = data4[0].suggested_allocation_quantity;
                                             var r = ((sq * m) + sq);
                                            pool.query(`UPDATE suggested_order SET suggested_allocation_quantity=${r} WHERE suggested_id = ${data4[0].suggested_id}`, (err, results, fields) => {
                                              if (err) {
                                                  return res.status(500).json({err});
                                              }
                                            });
                                      ...
                                      if (err){ 
                                       ...
                                       pool.query(`insert into total_suggestions SELECT product_id, product_name, sum(suggested_allocation_quantity) FROM suggested_order GROUP BY product_id`, function(err, results){
                                          if (err){ 
                                                    throw err;
                                            }
